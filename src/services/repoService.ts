...
export const parseRepoUrl = (url: string): { owner: string; repo: string; type: RepoType; path?: string } | null => {
  try {
    ...
    if (pathParts.length > 2) {
      ...
      const skipIndex = type === 'github' && (pathParts[2] === 'blob' || pathParts[2] === 'tree') ? 3 :
                        type === 'gitlab' && pathParts[2] === '-' && (pathParts[3] === 'blob' || pathParts[3] === 'tree') ? 4 : 2;
...
export const fetchBranches = async (
  ...
  try {
    if (type === 'github') {
      ...
      // GitLab API
      const response = await axios.get(
        `https://gitlab.com/api/v4/projects/${encodeURIComponent(`${owner}/${repo}`)}/repository/branches`
      );
      return response.data.map((branch: any) => branch.name);
    }
  } catch (error) {
    ...
    if (type === 'github') {
      ...
      const files: RepoFile[] = await Promise.all(
        response.data.map(async (item: any) => {
          ...
          if (file.type === 'dir') {
            file.children = await fetchRepoFiles(owner, repo, branch, type, file.path);
          }
...
export const saveRepoToExamples = (repoData: RepoData): void => {
  ...
  try {
    ...
    const existingIndex = existingSaved.findIndex(
      r => r.owner === savedRepo.owner && r.repo === savedRepo.repo && r.type === savedRepo.type
    );
...
export const removeSavedRepo = (owner: string, repo: string, type: RepoType): void => {
  try {
    const existingSaved = getSavedRepos();
