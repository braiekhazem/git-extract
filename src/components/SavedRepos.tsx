...
const SavedRepos: React.FC<SavedReposProps> = ({ savedRepos, onSelectRepo, onReposChange }) => {
  ...
  if (savedRepos.length === 0) {
    return (
      <div className="text-center text-muted-foreground py-4">
        ...
        <p className="text-sm">Explore repositories and save them to see them here.</p>
      </div>
    );
  }
      ...
      {savedRepos.map((repo) => (
        ...
        >
          <CardContent className="p-4 flex items-center justify-between">
            <div className="flex items-center gap-3" onClick={() => onSelectRepo(repo.url)}>
              {repo.type === 'github' ? (
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Saved {formatDistanceToNow(new Date(repo.savedAt))} ago
                </p>
              </div>
            </div>
